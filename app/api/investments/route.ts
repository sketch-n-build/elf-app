...
/* ─────────────────────────────────────────────────────────────
   GET /api/investments
   ADMIN/STAFF  → see all investments, with optional filters.
   INVESTOR     → sees only their own investments.
   Query params: page, limit, projectId, userId (admin only)
───────────────────────────────────────────────────────────── */
export const GET = apiAuth(async (req: NextRequest & { user: any }) => {
  ...
  try {
    ...
    // Investors are scoped to their own investments
    const userFilter =
      req.user.role === "INVESTOR"
        ? { userId: req.user.sub }
        : req.user.role === "ADMIN" && searchParams.get("userId")
...
/* ─────────────────────────────────────────────────────────────
   ...
   Creates an investment against an active project.
   Access: INVESTOR only (Admin/Staff invest through the dashboard)
───────────────────────────────────────────────────────────── */
export const POST = apiAuth(async (req: NextRequest & { user: any }) => {
  ...
  try {
    ...
    if (!projectId || !amount) {
      return NextResponse.json(
        { success: false, error: "projectId and amount are required." },
        { status: 400 }
      );
    }
    ...
    if (project.status !== "ACTIVE") {
      return NextResponse.json(
        { success: false, error: "Investments can only be made on active projects." },
        { status: 422 }
      );
    }
